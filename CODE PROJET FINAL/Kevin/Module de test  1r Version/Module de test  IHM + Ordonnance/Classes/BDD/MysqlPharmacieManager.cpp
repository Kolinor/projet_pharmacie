...
bool MysqlPharmacieManager::createOrdonnance(String numeroCaisse)
{
	...
	if (numeroCaisse==1){
		deriereCommandeCAISSE1="SELECT `ID_Ordonnance` FROM `Ordonnance` WHERE `ID_Ordonnance`= " + numeroCaisse + ";";
	}
	if (numeroCaisse==2){
...
String id(0);
	...
	String requeteOrdonnance="INSERT INTO `Ordonnance_Medicament_Association`(`ID_Ordonnance`, `Nom_Medicament`, `Quantite_Demande`,`Quantite_Delivree`) VALUES (" + id + ",'" + Nom_Medicament + "'," + quantite + ",0);";
	wchar_t * wStrReq = requeteOrdonnance.c_str();
	int reqLength = wcslen(wStrReq);
	char * req = new char[reqLength + 1];
	wcstombs(req, wStrReq, reqLength);
